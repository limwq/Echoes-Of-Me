...
public class GameSceneManager : MonoBehaviour {
    ...
    public struct SceneMusic {
        ...
        public string bgmName; // The name of the sound in your AudioManager
    }
    ...
    private IEnumerator LoadSceneRoutine(string sceneName) {
        ...
        // --- NEW: Play Music BEFORE fading in ---
    ...
    private void PlaySceneMusic(string sceneName) {
        // 1. Look through the playlist for a match
        foreach (SceneMusic track in musicPlaylist) {
            if (track.sceneName == sceneName) {
                // Found a match! Play the new song.
                if (AudioManager.Instance != null) {
                    AudioManager.Instance.PlayMusic(track.bgmName);
                }
                return; // Exit immediately so we don't hit the stop command below
            }
        }

        // 2. If we reach this point, it means the scene was NOT in the playlist.
        // Therefore, we should STOP the music (Silence).
        if (AudioManager.Instance != null) {
